...
class UserControllerTest {
    ...
    void testGetlist() throws Exception {
        ...
        List<User> testList  = (List<User>)result.getModelAndView().getModel().get("userlist");
        assertEquals(testList.size(),3);
        assertEquals(testList.get(0),1);
        assertEquals(testList.get(0).getId(), 1);
        assertEquals(testList.get(0).getName(), "キラメキ太郎");
        assertEquals(testList.get(1),2);
        assertEquals(testList.get(1).getId(), 2);
        assertEquals(testList.get(1).getName(), "キラメキ次郎");
        assertEquals(testList.get(2),3);
        assertEquals(testList.get(2).getId(), 3);
        assertEquals(testList.get(2).getName(), "キラメキ花子");
    }
}